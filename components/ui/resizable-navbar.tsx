      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-neutral-800/80"
            />
          )}
  ...
  return (
    ...
    >
      <Image
        ...
        height={30}
      />
      <span className="font-medium text-white dark:text-white">SKills Sphere</span>
    </Link>
  );
};
...
export const NavbarButton = ({
  ...
  href?: string;
  as?: "a" | "button" | "div"; // Limit to specific elements
  children: React.ReactNode;
  className?: string;
